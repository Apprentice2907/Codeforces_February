...
void solve() {
    ...
    int prefix = s_pos.front();
    int suffix = n - 1 - s_pos.back();

    int ans = (prefix + 1) / 2 + (suffix + 1) / 2;

    for (int i = 0; i + 1 < (int)s_pos.size(); i++) {
        int d = s_pos[i + 1] - s_pos[i];
        ans += (d - 1) / 2;
    }
