...
// Understood but claude coded

#include <iostream>
using namespace std;

int solve(long long a, long long b, bool whiteFirst) {
    int layers = 0;
    long long size = 1;
    
    while (true) {
        bool useWhite = (layers % 2 == 0) == whiteFirst;
...
int main() {
    ...
    while (t--) {
        ...
        int result1 = solve(a, b, true);   
        int result2 = solve(a, b, false); 
